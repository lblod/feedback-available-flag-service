...
const LDES_GRAPH = env.get('LDES_GRAPH').default('http://mu.semte.ch/graphs/lpdc/conceptsnapshots-ldes-data/ipdc-feedback').asString();
const HEALING_CRON = env.get('HEALING_CRON').default('0 3 * * *').asString();
const INGEST_CRON = env.get('INGEST_CRON').default('*/5 * * * *').asString();
