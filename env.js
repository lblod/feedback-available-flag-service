...
const UNKNOWN_GRAPH = env.get('UNKNOWN_GRAPH').default('http://mu.semte.ch/graphs/lpdc/feedbacksnapshot-ldes-data/unknown-receiver').asString();
const HEALING_CRON = env.get('HEALING_CRON').default('0 3 * * *').asString();
const INGEST_CRON = env.get('INGEST_CRON').default('*/1 * * * *').asString();
const PUBLISH_CRON = env.get('PUBLISH_CRON').default('*/10 * * * *').asString();
